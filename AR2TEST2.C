...
void ttest_inxtprm_i(char *e, char *b);
...
void main()
  ...
  ttest_ieval("20017*666*(7*(6^(2^(3^(3*3)-19680))+7)*(2+(91*2^10-3)*2)+2000)*23^(9+2)+2^(432-(3*6)+(6797/92+29*(2^1+9)+2))", 
  ...
  imovd(a, "0"); dd = ilog(a); dl = dd; imovk(b, dl); 
  tcheckd(b, "0", "ilog", 0);
  imovd(a, "1"); dd = ilog(a); dl = dd; imovk(b, dl); 
  tcheckd(b, "0", "ilog", 1);
  imovd(a, "2"); dd = ilog(a); dl = dd; imovk(b, dl); 
  tcheckd(b, "0", "ilog", 2);
  imovd(a, "3"); dd = ilog(a); dl = dd; imovk(b, dl); 
  tcheckd(b, "1", "ilog", 3);
  imovd(a, "4"); dd = ilog(a); dl = dd; imovk(b, dl); 
  tcheckd(b, "1", "ilog", 4);
  imovd(a, "5"); dd = ilog(a); dl = dd; imovk(b, dl); 
  tcheckd(b, "1", "ilog", 5);
  imovd(a, "0"); isqrt(b, a); imov(c, res);
  tcheckd(b, "0", "isqrt", 0); tcheckd(c, "0", "isqrt res", 0);
  imovd(a, "1"); isqrt(b, a); imov(c, res);
  tcheckd(b, "1", "isqrt", 1); tcheckd(c, "0", "isqrt res", 1);
  imovd(a, "2"); isqrt(b, a); imov(c, res);
  tcheckd(b, "1", "isqrt", 2); tcheckd(c, "1", "isqrt res", 2);
  imovd(a, "3"); isqrt(b, a); imov(c, res);
  tcheckd(b, "1", "isqrt", 3); tcheckd(c, "2", "isqrt res", 3);
  imovd(a, "4"); isqrt(b, a); imov(c, res);
  tcheckd(b, "2", "isqrt", 4); tcheckd(c, "0", "isqrt res", 4);
  imovd(a, "5"); isqrt(b, a); imov(c, res);
  tcheckd(b, "2", "isqrt", 5); tcheckd(c, "1", "isqrt res", 5);
  imovd(a, "65535"); isqrt(b, a); imov(c, res);
  tcheckd(b, "255", "isqrt", 65535); tcheckd(c, "510", "isqrt res", 65535);
  imovd(a, "65536"); isqrt(b, a); imov(c, res);
  tcheckd(b, "256", "isqrt", 65536); tcheckd(c, "0", "isqrt res", 65536);
  imovd(a, "65537"); isqrt(b, a); imov(c, res);
  tcheckd(b, "256", "isqrt", 65537); tcheckd(c, "1", "isqrt res", 65537);
  ttest_ilog_isqrt(10);
  ttest_ilog_isqrt(21);
  ttest_ilog_isqrt(32);
...
void ttestigcdext(char *cx, char *cy)
  ...
  for (i = 1; i <= 100; i++)
    ...
    imulk(x, 20); iaddk(x, 23);
    imulk(y, 33); iaddk(x, 47);
    }
  }
...
void ttestiexp(char *cx, char *cy, char *cz)
  ...
  for (sx = -1; sx <= 1; sx++)
    ...
    for (sy = -1; sy <= 1; sy++)
      {
      imovd(x, cx); imulk(x, sx);
      imovd(y, cy); imulk(y, sy);
      imovd(z, cz);
      iexp(a, x, y);           /* a = x^y */
      imov(b, a);
...
void ttest_ilog_isqrt(long k)
  ...
  for (i = 0; i < 10; i++)
    ...
    imovk(b, dl - el); 
    tcheckd(b, "0", "ilog", dl);
    imov(b, a); iaddk(b, -2); isqrt(c, b); imov(d, res); isqu(c); iadd(c, d); isub(c, b);
    tcheckd(c, "0", "isqrt -2", 100*k+i);
    imov(b, a); iaddk(b, -1); isqrt(c, b); imov(d, res); isqu(c); iadd(c, d); isub(c, b);
    tcheckd(c, "0", "isqrt -1", 100*k+i);
    imov(b, a); isqrt(c, b); imov(d, res); isqu(c); iadd(c, d); isub(c, b);
    tcheckd(c, "0", "isqrt 0 ", 100*k+i); if (i > 1) tcheckd(d, "0", "isqrt res", 100*k+i); 
    imov(b, a); iaddk(b, 1); isqrt(c, b); imov(d, res); isqu(c); iadd(c, d); isub(c, b);
    tcheckd(c, "0", "isqrt +1", 100*k+i);
    imov(b, a); iaddk(b, 2); isqrt(c, b); imov(d, res); isqu(c); iadd(c, d); isub(c, b);
    tcheckd(c, "0", "isqrt +2", 100*k+i);
...
void ttest_ieval(char *e, char *b)
  {
  imov(a, ieval(e));
  if (*eval_err != NULL) tprint("Error: err=%s \n", eval_err);
  tcheckd(a, b, "ieval", 0);
  if (*eval_err != NULL) halt();
  } 
