...
static unsigned char *ar2p_pr, *ar2p_pr_lo = ar2p_pr_tab, *ar2p_pr_hi = ar2p_pr_tab + pr_tab_dim;
static unsigned long ar2p_es_tab[es_tab_dim];
static unsigned long *ar2p_es, *ar2p_es_lo = ar2p_es_tab, *ar2p_es_hi = ar2p_es_tab + es_tab_dim;
static ireg ar2p_rp_lo, ar2p_rp_hi; 
static ireg* ar2p_ep_lo = &ar2p_rp_lo; 
static ireg* ar2p_ep_hi = &ar2p_rp_hi; 
static long ar2p_f = 0;

void ar2p_get_primes()
  {
  /*
     Use the Sieve of Eratosthenes to set up es_tab for the
     next batch of primes, p_lo <= p < p_lo + 64*w.
  */
  static unsigned long f, k, k_lo, k_hi, q, q_hi;
  static rrx; 
  static ireg* rx = &rrx; 
  ...
  q_hi = iwd0(rx) + 2;
  f = 0;
  for (ar2p_es = ar2p_es_lo; ar2p_es < ar2p_es_hi; ar2p_es++) *ar2p_es = 0;
  for (q = 3, ar2p_pr = ar2p_pr_lo; q < q_hi; q += *ar2p_pr, ar2p_pr++)
    ...
    if (k_lo == q) k_lo = 0;
      ...
      {
      if (!(k_lo & 1)) k_lo >>= 1;
      else k_lo = (k_lo + q) >> 1;     /* k_lo = -p/2 (mod q) */
      }
  ...
  printf("AR2P: %d bits set in [", f); 
  idisp(ar2p_ep_lo); printf(" "); idisp(ar2p_ep_hi); 
  printf(") with sieve limit %d \n", q_hi);
  ar2p_f = 1;
  }

void ar2p_check_pr_tab()
  {
  static unsigned long p, q;
  ...
  for (pr = ar2p_pr_lo, p = 3, q = 3; pr < ar2p_pr_hi; pr++)
    ...
    if (p != q)
      {
      printf("AR2P: Error at %d in pr_tab: %d should be %d\n",
             (pr - ar2p_pr_lo), p, q);
      stop();
      }
    }  
    ...
    for (r = 0, w = *ar2p_es; r < 32; r++, w >>= 1, q += 2)
      ...
      if (!(w & 1))
        ...
        *qr = q - q0;
        ...
        if (qr >= ar2p_pr_hi)
          {
          printf("AR2P: base=3 p[0]=%d p[1]=%d p[2]=%d ... p[%d]=%d p[%d]=%d \n",
