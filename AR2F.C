...
void fexpmod(ireg *a, ireg *b, ireg *c, ireg *q, void (fsqu)(ireg *a))
  ...
  */
  ...
  if (b->value == NULL) { iinvparm("fexpmod"); }
  if (b->digits < 1)    { iinvparm("fexpmod"); }
  if (c->value == NULL) { iinvparm("fexpmod"); }
  if (c->digits < 1)    { iinvparm("fexpmod"); }
  imovk(a, 1);
  if (isgn(c) <= 0) {return; }
  f = 0;
...
void fexpmodm2ke(ireg *a, ireg *b, ireg *c, ireg *q, long m, long k, ireg *e, void (fsqu)(ireg *a))
  ...
  */
  ...
  static long i, f;
  static unsigned long j;
  if (b->value == NULL) { iinvparm("fexpmodm2ke"); }
  if (b->digits < 1)    { iinvparm("fexpmodm2ke"); }
  if (c->value == NULL) { iinvparm("fexpmodm2ke"); }
  if (c->digits < 1)    { iinvparm("fexpmodm2ke"); }
  if (q->value == NULL) { iinvparm("fexpmodm2ke"); }
  if (q->digits < 1)    { iinvparm("fexpmodm2ke"); }
  if (isgn(q) <= 0)     { iinvparm("fexpmodm2ke"); }
  imovk(a, 1);
  if (isgn(c) <= 0) {return; }
  f = 0;
  for (i = c->digits - 1; i >= 0; i--)
